...
- **Round 2, large spaces ([`large/`](large/README.md)):** AGS beats random search clearly on big smooth synthetic grids but trails Optuna. On a real 103,680-config boosting grid it came last (test AUC 0.8808 vs random 0.8832 vs Optuna 0.8878). Default early stopping stops it after about 20 evaluations. Grids of about 10⁸ points run out of memory. Root cause of the losses: in 8 dimensions it never looked more than 3 steps from its current best, so it climbs whichever hill it starts on.
